...
class Statements():
    ...
    def eval(self, context):
        if self.values == [None] and self.value is None:
            return
        if self.values == [None]:
            return [self.value.eval(context)]
        elif self.value is None:
            return [v.eval(context) for v in self.values]
...
class IfStatement():
    def __init__(self, boolean, body, else_body):
